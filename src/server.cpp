...
#if 0
bool OmicroServer::initTrxn( OmicroTrxn &txn )
{
...
void OmicroServer::multicast( OmicroClient &client,  const strvec &hostVec, const sstr &trxnMsg, bool expectReply, strvec &replyVec )
{
	sstr id, ip, port;
	int len = hostVec.size();
	if ( len < 1 ) {
	...
	for ( int i=0; i < len; ++i ) {
		...
		// pthread_create(&thrd[i], NULL, &threadSendMsg, (void *)&thrdParam[i]);
		// threadSendMsg((void *)&thrdParam[i]);
		client.connect( ip.c_str(), thrdParam[i].port);
		thrdParam[i].reply = client.sendMessage( trxnMsg.c_str(), 300);
		client.disconnect();
	}
...
void omsession::do_read()
{
    ...
    socket_.async_read_some(boost::asio::buffer(data_, max_length),
          [this, self](boost::system::error_code ec, std::size_t length)
          {
            ...
            if (!ec)  // no error
            {
                d("a82828 read_some data_[%s]", data_ );
                sstr msg(data_);
                callback( msg );
            } else {
...
void omsession::reply( const sstr &str )
{
    ...
    boost::asio::async_write(socket_, boost::asio::buffer(str.c_str(), str.size() ),
          [this, self, &str](boost::system::error_code ec, std::size_t)
          {
            if (!ec)  // no error, OK
            {
                // do_read();
