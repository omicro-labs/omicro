...
{
    d("a4421 OmicroClient::sendMessage(msg=%.200s) msglen=%d expectReply=%d", s(msg), msg.size(), expectReply );
    if ( ! connectOK_ ) {
    ...
    sstr zip;
    if ( msg.size() > doCompressThreshold ) {
    ...
    if ( ! expectReply ) { 
        d("a00020 not expectReply return empty");
        return ""; 
    }
    ...
    while ( 1 ) {
        ...
        d("c00884 got reply from server = [%s]", s(res) );
...
#if 0
// need to do second  QQ call, which is after sleep(5) and unreliable
...
sstr OmicroClient::reqPublicKey( int waitSeconds)
{
    ...
    while ( true ) {
    	...
    	if ( resp.STT_ == OM_RESP_OK ) {
    		d("a32038 recved pubkey=[%.100s]", s(resp.DAT_) );
    		return resp.DAT_;
    	}
